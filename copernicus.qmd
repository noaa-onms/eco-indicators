...
for (i in 1:nrow(sanctuaries)) {
  ...
  for (j in 1:length(datasets)) {
    ...
    ds <- datasets[[j]]
    message(glue("  {j}/{length(datasets)}: {ds_id}", .trim = F))
    ...
    message(glue("  : end ~ {format(Sys.time(), '%H:%M:%S')}", .trim = F))
  }
}

# 20 min to complete:
#  1/15: Cordell Bank (CBNMS) : beg                 ~ 22:42:10
# 15/15: Monterey Bay - Mainland (MBNMS-main) : end ~ 23:02:34
```
